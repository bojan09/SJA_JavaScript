// storage - Механизам за зачувување на податоци

localStorage.setItem("ime", "Bojan");
const ime = localStorage.getItem("ime"); // It will return "Bojan"
console.log(ime);
...
const addAnimals = () => {
  ...
  animals = animals.map((animal) => {
    if (animal.type === newAnimal.type) {
      ...
      return {
        ...animal,
        number: Number(animal.number) + Number(newAnimal.number),
      };
    }
    return animal;
  });
